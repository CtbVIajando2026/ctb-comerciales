...
export async function obtenerNotificaciones(comercialId: string): Promise<Notificacion[]> {
  try {
    ...
    try {
      ...
      console.error('[Notificaciones] Error en seguimientos:', e)
    }

    // 2. Cumpleaños de contactos - Top 3 próximos, filtrados por ciudad
    try {
      // Fix: filter by ciudad using a separate query to avoid Supabase inner-join filter bug
      const { data: agenciasCiudad } = await supabase
        .from('agencias')
        .select('id')
        .eq('activa', true)
        .eq('ciudad', ciudadComercial)
      
      const agenciasIds = (agenciasCiudad || []).map((a: any) => a.id)

      if (agenciasIds.length > 0) {
        const { data: contactos } = await supabase
          ...
          .select(`
            ...
            nombre,
            fecha_cumpleanos,
            agencia:agencias(id, nombre, ciudad)
          `)
          .eq('activo', true)
          .in('agencia_id', agenciasIds)
        ...
        if (contactos) {
          contactos.forEach(cont => {
            ...
            proximosCumples.push({ data: cont, fechaCalculada: cumpleEsteAno })
          })
      ...
      console.error('[Notificaciones] Error en cumpleaños:', e)
    }
    ...
    try {
      ...
      if (agencias) {
        agencias.forEach(ag => {
          ...
          proximosAniv.push({ data: ag, fechaCalculada: anivEsteAno })
        })
      ...
      console.error('[Notificaciones] Error en aniversarios:', e)
    }
    ...
    console.error('[Notificaciones] Error general:', e)
    return []
  }
}
