...
import { MapPin, Sun, Moon, Loader2, Bell } from 'lucide-react'
import { cn } from '@/lib/utils'
import Link from 'next/link'
import { ThemeToggle } from '@/components/shared/ThemeToggle'

export function LiveLocationHeader({ perfil, notificacionesCount = 0 }: { perfil: any, notificacionesCount?: number }) {
    ...
    <div className="w-full relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#0f172a] via-[#1e1b4b] to-[#312e81] text-white shadow-2xl shadow-indigo-900/20 mb-6 p-6 sm:p-8">
      ...
      <div className="relative z-10 flex flex-col gap-6">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right flex flex-col items-end gap-3 z-20">
            <div className="flex items-center gap-2">
              <ThemeToggle />
              <Link href="/comerciales/notificaciones" className="relative group">
                <div className="p-2.5 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-colors backdrop-blur-md">
                  <Bell className="w-5 h-5 text-indigo-100 group-hover:text-white transition-colors" />
                  {notificacionesCount > 0 && (
                    <span className="absolute -top-1 -right-1 w-4 h-4 bg-destructive text-destructive-foreground text-[9px] font-bold flex items-center justify-center rounded-full shadow-sm animate-pulse border border-[#1e1b4b]">
                      {notificacionesCount}
                    </span>
                  )}
                </div>
